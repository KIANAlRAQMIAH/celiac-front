...
import { clearUserData, setToken } from '../../api/AuthSlice';


const Header = () => {
    const { userData, permissions } = useSelector((state: any) => state.auth);
                ...
                <div className="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                        <Link to="/home" className="main-logo flex items-center shrink-0">
                            <img className="w-36 ltr:-ml-1 rtl:-mr-1 inline" src="../../../public/logo512.png" alt="logo" />
                            ...
                            {themeConfig.theme === 'light' ? (
                                <button
                                    className={`${themeConfig.theme === 'light' &&
                                        'flex items-center p-2 mr-[11px] rounded-full  bg-white-light/40 dark:bg-dark/40 hover:text-primary hover:bg-white-light/90 dark:hover:bg-dark/60'
                                        }`}
                                    onClick={() => {
