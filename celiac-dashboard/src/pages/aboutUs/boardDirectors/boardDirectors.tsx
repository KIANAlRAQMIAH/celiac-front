...
const BoardDirectors = () => {
    ...
    //   ...
    const { data, status, isLoading }: any = useGetBoardMembersQuery({ page: currentPage, keyword: search });
    ...
    const [addBoardMember, { isLoading: isLoadingGet }] = useAddBoardMemberMutation();
    const [deleteMemberById] = useDeleteMemberByIdMutation();
    const [updateBoardMember, { isLoading: isLoadingEdit }] = useUpdateBoardMemberMutation();
    ...
    useEffect(() => {
        dispatch(setPageTitle('أعضاء الجمعيه'));
    });
    ...
    if (!canRead('BoardDirectorMember')) {
        return <Error404 />;
    }
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl">أعضاء مجلس الاداره</h2>
                <div className="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                    <div className="flex gap-3">
                        <div>
                            {canCreate('BoardDirectorMember') &&
                                    ...
                                    <Dialog.Panel as="div" className=" panel border-0 p-5 rounded-lg overflow-hidden w-full max-w-lg my-8 text-black dark:text-white-dark">
                                        ...
                                        <div className="p-5">
                                            <form className="space-y-5" onSubmit={handleSubmit}>
                                                ...
                                                <div>
                                                    <label htmlFor="positionId">الوظيفة</label>
                                                    <select
                                                        id="positionId"
                                                        value={positionId}
                                                        onChange={(e) => {
    ...
    <Fragment>
        {image && (
            <img src={imageUrl} className="w-[100%] object-cover ltr:mr-2 rtl:ml-2" alt="avatar" />
        )}
        {image && (
            <img src={image.url} className="w-[100%] object-cover ltr:mr-2 rtl:ml-2" alt="avatar" />
        )}
    </Fragment>
) : (
    imageUrl && (
