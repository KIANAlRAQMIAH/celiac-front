...
import { useAddGovernanceFilesMutation, useDeleteGovernanceFileByIdMutation, useGetGovernanceFilesQuery, useGetGovernanceListsQuery, useUpdateGovernanceFileMutation } from '../../../api/AboutUsSlice/GovernanceSlice';
...
const Governance = () => {
    ...
    //   ...
    const { id } = useParams();
    const { data: listGovernanceData } = useGetGovernanceListsQuery()
    const { data, status, isLoading }: any = useGetGovernanceFilesQuery(id);
    const [addOrganizationfile, { isLoading: isLoadingGet }] = useAddGovernanceFilesMutation();
    const [deletefileById] = useDeleteGovernanceFileByIdMutation();
    const [updateOrganizationfile, { isLoading: isLoadingEdit }] = useUpdateGovernanceFileMutation();
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (id !== undefined) {
                formData.append('governance_list_id', id);
            }
            if (file) {
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                {listGovernanceData?.data?.map((item: any) => {
                    if (item.id == id) {
                        return <h2 className="text-xl" key={item.id}>{item?.name}</h2>;
                    }
                    return null;
                })}
