...
import {
    ...
    useDeleteOrganizationMemberByIdMutation,
    useGetOrganizationMembersQuery,
    useUpdateOrganizationMemberMutation,
} from '../../../api/AboutUsSlice/OrganizationChartSlice';
...
import { useGetPositionsQuery } from '../../../api/AboutUsSlice/CommitteesSlice';
...
const OrganizationChart = () => {
    ...
    // @ts-ignore
    const { data, status, isLoading }: any = useGetOrganizationMembersQuery(currentPage);
    const { data: positionsData, isLoading: isLoadingPositions }: any = useGetPositionsQuery();
    ...
    const [updateOrganizationMember, { isLoading: isLoadingEdit }] = useUpdateOrganizationMemberMutation();
    ...
    const { data: ann }: any = useGetGovernanceListsQuery()
    useEffect(() => {
        if (MemberList) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (editing && memberId) {
                ...
                const response = await addOrganizationMember(formData).unwrap();
    ...
    if (!canRead('OrganizationalStructureMember')) {
        return <Error404 />;
    }
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl">الهيكل التنظيمي</h2>
                <div className="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                    <div className="flex gap-3">
                        <div>
                            {canCreate('OrganizationalStructureMember') &&
                                    ...
                                    <Dialog.Panel as="div" className=" panel border-0 p-5 rounded-lg overflow-hidden w-full max-w-lg my-8 text-black dark:text-white-dark">
                                        ...
                                        <div className="p-5">
                                            <form className="space-y-5" onSubmit={handleSubmit}>
                                                ...
                                                <div>
                                                    <label htmlFor="positionId">نوع الوظيفه</label>
                                                ...
                                                <div>
                                                    <label htmlFor="image">تحميل الصوره (.png only)</label>
                                                    <input
                                                        id="image"
                                                        type="file"
