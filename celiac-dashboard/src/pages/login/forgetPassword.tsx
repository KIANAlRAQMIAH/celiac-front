...
const ForgetPassword = () => {
    const [userData, { data, isSuccess, isLoading, error, isError, status }] = useForgetPasswordMutation();
    const {
        register,
        handleSubmit,
        watch,
    ...
    const submitForm = async (e: any) => {
        const data = await userData(e);
        //@ts-ignore
        if (data?.data?.status === 200) {
            navigate('/auth/otp', { state: { email: e.email } });
        }
        //@ts-ignore
    ...
    //     if (isSuccess) {
    //         // showAlert("Delete");
    //         navigate('/auth/otp', { state: { email: 'amramr' } });
    //     }
    // }, [isSuccess]);
