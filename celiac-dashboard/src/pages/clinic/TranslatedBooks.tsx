...
const TranslatedBooks = () => {
    ...
    //   ...
    const { data, status, isLoading }: any = useGetTranslatedBooksQuery(currentPage);
    ...
    useEffect(() => {
        dispatch(setPageTitle('الكتب المترجمه'));
    });
    ...
    if (!canRead('TranslatedBook')) {
        return <Error404 />;
    }
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl">الكتب المترجمه</h2>
                <div className="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                    <div className="flex gap-3">
                        <div>
                            {canCreate('TranslatedBook') && <button type="button" className="btn btn-primary" onClick={addfile} >
                                اضافة بحث جديد
                            </button>
