...
const HiringModal = ({ modal2, setModal2, user }: Props) => {
    return (
                            ...
                            <Dialog.Panel as="div" className="panel border-0 p-5 rounded-lg overflow-hidden w-max my-8 text-black dark:text-white-dark">
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="col-span-1 bg-[#F5FBFC] p-4 rounded">
                                        {user?.cv?.ext !== 'pdf' ? <img src={user?.cv?.url} className="rounded w-full h-[150px] object-cover" alt="" /> : <FaUser size={130} className="w-full mb-4" />}
                                        <p className="text-[#001F15] text-[20px] font-bold">{user.full_name}</p>
                                        <p className="text-[#45534E] text-[16px] font-400">{user.dob}</p>
                                        <p className="text-[#45534E] text-[16px] font-400"><span className="font-bold">تاريخ الانضمام: </span>{user.created_at}</p>
                                    </div>
                                    <div className="col-span-2 pt-0 rounded">
                                        ...
                                        <div className="grid grid-cols-2 gap-[35px]">
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-2"><p className="font-bold">الهوية الوطنية</p></div>
                                                <p className="">{user.national_id}</p>
                                            </div>
                                            {user.civil_id && (
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-2"><p className="font-bold">الهوية المدنية</p></div>
                                                    <p className="">{user.civil_id}</p>
                                                </div>
                                            )}
                                            {user.residency_number && (
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-2"><p className="font-bold">رقم الإقامة</p></div>
                                                    <p className="">{user.residency_number}</p>
                                                </div>
                                            )}
                                        </div>
                                        {user.cv && (
                                            <NavLink to={user.cv.url} className="rounded-lg border p-4 my-4 flex justify-between items-center">
                                                <div className="flex items-center gap-2">
                                                    <img src="../../../../public/assets/images/pdfLofo.svg" alt="" />
                                                    <p className="">{user.cv.name}</p>
                                                </div>
