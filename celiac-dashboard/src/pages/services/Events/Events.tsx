...
import SessionsForm from './EventsForm';
...
export default function Events() {
    ...
    const deleteSubmitHandler = useDeleteConfirmation({ url: 'events', inValid: 'events' });
    ...
    return (
        <Main_list title="الفعاليات">
            ...
            {open && !editData?.id &&!viewId && (
                <CustomModal openCloseModal={setOpen} title="اضافة فعاليه">
                    <SessionsForm openCloseModal={setOpen} editData={null} />
                </CustomModal>
            )}

            {open && editData?.id && !viewId && (
                <CustomModal openCloseModal={setOpen} title={`عرض تفاصيل الفعاليه`} resetEditData={setEditData}>
                    <EventsForm editData={editData} resetEditData={setEditData} openCloseModal={setOpen} />
                </CustomModal>
            )}
            {open && viewId && !editData?.id && (
                <CustomModal openCloseModal={setOpen} title={`عرض تفاصيل الفعاليه`} setViewId={setViewId}>
                    <EventsDetails viewId={viewId}   openCloseModal={setOpen} setEditData={setEditData} />
                </CustomModal>
            )}

            {open && editData?.id && (
                <CustomModal openCloseModal={setOpen} title="تعديل الفعاليه" resetEditData={setEditData}>
                    <EventsForm editData={editData} resetEditData={setEditData} openCloseModal={setOpen} />
                </CustomModal>
            )}
             <ColumnChooser
        ...
        tabelHead = {[
            ...
            { accessor: "name", title: "اسم الفعاليه" },
            { accessor: "session_date", title: "تاريخ الفعاليه" },

            { accessor: "starts_at", title: "وقت البدء" },
            { accessor: "ends_at", title: "وقت الانتهاء" },
            { accessor: "event_type_text", title: "نوع الفعاليه" }, // Adjusted to match the provided object
