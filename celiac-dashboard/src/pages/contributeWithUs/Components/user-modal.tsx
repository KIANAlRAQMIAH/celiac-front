...
import { useGetSingleVolunteerQuery } from "../../../api/serveces";
...
const UserModal = ({ modal2, setModal2, id }: Props) => {
    const { data, isLoading } = useGetSingleVolunteerQuery(id);
    return (
                            ...
                            <Dialog.Panel as="div" className="panel border-0 p-5 rounded-lg overflow-hidden w-max my-8 text-black dark:text-white-dark">
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="col-span-1 bg-[#F5FBFC] p-4 rounded">
                                        {data?.data.cv?.url ? <img src={data?.data.cv?.url} className="rounded w-full h-[150px] object-cover" alt="" /> : <FaUser size={130} className="w-full mb-4" />}
